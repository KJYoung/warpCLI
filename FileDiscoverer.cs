...
namespace Warp
{
    public class FileDiscoverer
    {
        ...
        public FileDiscoverer()
        {
            ...
            //Console.WriteLine("FileDiscoverer successfully generated!");
        }
        ...
        void WorkLoop(object sender, EventArgs e)
        {
            ...
            //Console.WriteLine("WorkLoop of FileDiscoverer.cs is called. Finding {0} in {1}", FileExtension, FolderPath);
            ...
            if(false){
                ...
                {
                    if (File.Extension.ToLower().CompareTo(".tomostar") == 0)
                    {
                ...
                Console.WriteLine("is ShouldAbort true? : {0}", ShouldAbort);
            }
            while (true)
            {
                if (ShouldAbort)
                                        ...
                                        if (!Ripe.Exists(m => m.Path == fileName))
                                        {
                                            ...
                                            Task CreationTask = new Task(() =>
                                            {
                                                ...
                                                if (!fileName.Substring(fileName.Length - 8).ToLower().Contains("tomostar")){
                                                    ...
                                                    Console.WriteLine("We call Tilt Constructor");
                                ...
                                if (!Ripe.Exists(m => m.Path == fileName))
                                {
                                    ...
                                    Task CreationTask = new Task(() =>
                                    {
                                        ...
                                        if (!fileName.Substring(fileName.Length - 8).ToLower().Contains("tomostar"))
                                            ...
                                            Created = new TiltSeries(fileName);
                                            //Console.WriteLine("Result : {0} ||||| {1} ", ((TiltSeries)Created).crd, Created.info);
                                        }    
                                        lock (Ripe)
                                        {
                                            // Make sure the list is sorted
                                            ...
                                            //Console.WriteLine("Ripe insert2 is valid : {0}, {1}", InsertAt, Created);
                                            Ripe.Insert(InsertAt, Created);
                                        }
